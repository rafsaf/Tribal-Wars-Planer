...
def new_outline_create(request):
    ...
    if request.method == "POST":
        ...
        form1.fields["world"].choices = [
            (f"{i.pk}", f"{str(i)}")
            for i in models.World.objects.filter(server=request.user.profile.server).order_by("pk")
        ]
        if form1.is_valid():
...
def new_outline_create_select(request, _id):
    ...
    ally_tribe = [tribe for tribe in models.Tribe.objects.filter(world=instance.world, tag__in=instance.ally_tribe_tag)]
    enemy_tribe = [tribe for tribe in models.Tribe.objects.filter(world=instance.world, tag__in=instance.enemy_tribe_tag)]

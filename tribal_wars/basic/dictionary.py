...
def coord_to_player(outline: models.Outline):
    ...
    ally_villages = models.VillageModel.objects.select_related().filter(player__tribe__tag__in=outline.ally_tribe_tag, world=outline.world).values("coord", "player__name")
    village_dictionary = {}
...
def coord_to_player_from_string(village_coord_list: str, world: models.World):
    ...
    villages = models.VillageModel.objects.select_related().filter(world=world, coord__in=village_list).values("coord", "player__name")

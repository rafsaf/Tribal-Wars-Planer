...
class WriteRamTarget:
    ...
    def _get_filtered_weight_max_list(self):
        def all_filters_match(weight_max: WeightMaximum):
            for filter_func in self.filters:
                if filter_func(weight_max) is False:
                    return False
            return True
        ...
        return [
            ...
            if all_filters_match(weight) is True
        ]
    ...
    def _add_night_bonus_annotations(self):
        ...
        for weight_max in self.weight_max_list:
            time_hours: int = getattr(weight_max, "distance") / dividier
            time_mod = time_hours % 24
            night_score = (avg_dist - time_mod + 24) % 24
            if night_score >= 7 + interval_dist and night_score <= 24 - interval_dist:
                score = 3
            elif night_score >= 7 and night_score <= 24:
                score = 2
            else:
                score = 1
    ...
    def _first_line_false_query(self):
        def filter_first_line_false(weight_max: WeightMaximum):
            if (
                weight_max.first_line is False

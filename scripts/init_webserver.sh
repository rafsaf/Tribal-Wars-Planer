...
uwsgi --chdir=/build --uid=${SERVICE_NAME} --gid=${SERVICE_NAME} --module=tribal_wars_planer.wsgi:application --master --pidfile=/tmp/project-master.pid --socket=/tmp/uwsgi.sock --processes=${UWSGI_PROCESSES} --threads=2 --socket-timeout=150 --http-timeout=150 --harakiri=150 --home=/venv/ --daemonize=/build/logs/uwsgi.log
